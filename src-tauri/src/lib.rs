...
struct StepInput { title: String, description: String, command: Option<String>, requires_evidence: bool }
#[derive(Debug, Deserialize)]
...
fn db() -> Result<Connection, String> {
  ...
  conn.execute_batch("PRAGMA foreign_keys = ON;
    ...
    CREATE TABLE IF NOT EXISTS steps (id INTEGER PRIMARY KEY, procedure_id INTEGER NOT NULL REFERENCES procedures(id) ON DELETE CASCADE, order_index INTEGER NOT NULL, title TEXT NOT NULL, description TEXT NOT NULL DEFAULT '', command TEXT, requires_evidence INTEGER NOT NULL DEFAULT 0);
...
fn procedure(conn: &Connection, id: i64) -> Result<Procedure, String> {
  ...
  let mut statement = conn.prepare("SELECT id,procedure_id,order_index,title,description,command,requires_evidence FROM steps WHERE procedure_id=?1 ORDER BY order_index").map_err(|e| e.to_string())?;
  p.steps = statement.query_map([id], |r| Ok(Step { id:r.get(0)?, procedure_id:r.get(1)?, order_index:r.get(2)?, title:r.get(3)?, description:r.get(4)?, command:r.get(5)?, requires_evidence:r.get(6)? })).map_err(|e| e.to_string())?.collect::<Result<Vec<_>,_>>().map_err(|e| e.to_string())?; Ok(p)
}
...
#[tauri::command]
fn save_procedure(input: ProcedureInput) -> Result<Procedure, String> { if input.name.trim().is_empty() || input.steps.is_empty() || input.steps.iter().any(|s|s.title.trim().is_empty()) { return Err("Quy trình cần có tên và ít nhất một bước hợp lệ.".into()); } let mut conn=db()?; let tx=conn.transaction().map_err(|e|e.to_string())?; let stamp=now(); let id=match input.id.filter(|id|*id>0) { Some(id)=> { tx.execute("UPDATE procedures SET name=?1,description=?2,category=?3,updated_at=?4 WHERE id=?5",params![input.name,input.description,input.category,stamp,id]).map_err(|e|e.to_string())?; tx.execute("DELETE FROM steps WHERE procedure_id=?1",[id]).map_err(|e|e.to_string())?; id }, None=> { tx.execute("INSERT INTO procedures(name,description,category,created_at,updated_at) VALUES(?1,?2,?3,?4,?4)",params![input.name,input.description,input.category,stamp]).map_err(|e|e.to_string())?; tx.last_insert_rowid() } }; for (i,s) in input.steps.iter().enumerate() { tx.execute("INSERT INTO steps(procedure_id,order_index,title,description,command,requires_evidence) VALUES(?1,?2,?3,?4,?5,?6)",params![id,i as i64,s.title,s.description,s.command,s.requires_evidence]).map_err(|e|e.to_string())?; } tx.commit().map_err(|e|e.to_string())?; procedure(&conn,id) }
#[tauri::command]
...
#[tauri::command]
fn get_run(run_id: String) -> Result<RunDetails, String> { let conn=db()?; let run=conn.query_row("SELECT id,procedure_id,status,started_at,completed_at FROM runs WHERE id=?1",[&run_id],|r|Ok(Run{id:r.get(0)?,procedure_id:r.get(1)?,status:r.get(2)?,started_at:r.get(3)?,completed_at:r.get(4)?,procedure_name:None,confirmed_count:None,evidence_count:None})).map_err(|e|e.to_string())?; let procedure=procedure(&conn,run.procedure_id)?; let executions=conn.prepare("SELECT id,run_id,step_id,confirmed_at,notes,evidence_path,captured_at FROM step_executions WHERE run_id=?1").map_err(|e|e.to_string())?.query_map([&run_id],|r|Ok(Execution{id:r.get(0)?,run_id:r.get(1)?,step_id:r.get(2)?,confirmed_at:r.get(3)?,notes:r.get(4)?,evidence_path:r.get(5)?,captured_at:r.get(6)?})).map_err(|e|e.to_string())?.collect::<Result<Vec<_>,_>>().map_err(|e|e.to_string())?; Ok(RunDetails {run,procedure,executions}) }
#[tauri::command]
