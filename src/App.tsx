...
export default function App() {
  ...
  const removeProcedure = async () => { if (!editor.id) return; if (!window.confirm('Xóa quy trình này khỏi danh sách?\n\nLịch sử và ảnh bằng chứng của các lần đã chạy vẫn được giữ nguyên và xem lại được trong Lịch sử.')) return; try { setBusy(true); await api.deleteProcedure(editor.id); await loadProcedures(); goPicker(); } catch (e) { setNotice(String(e)); } finally { setBusy(false); } };
  ...
  return <main className="widget-app" style={widgetStyle}>
    ...
    <div className="widget-body">
      ...
      {view === 'runner' && details && <Runner details={details} busy={busy} setBusy={setBusy} onReload={async () => setDetails(await api.getRun(details.run.id))} onCancelRun={async () => { await api.setRunStatus(details.run.id, 'cancelled'); goPicker(); }} onFinished={(done) => { setFinished(done); setView('done'); }} onError={setNotice} />}
...
function Runner({ details, busy, setBusy, onReload, onCancelRun, onFinished, onError }: { details: RunDetails; busy: boolean; setBusy: (busy: boolean) => void; onReload: () => Promise<void>; onCancelRun: () => Promise<void>; onFinished: (details: RunDetails) => void; onError: (message: string) => void }) {
